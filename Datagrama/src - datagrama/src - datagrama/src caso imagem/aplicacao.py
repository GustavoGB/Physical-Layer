...
while True:
    ...
    ack = bytes([0,0,2,8])
    nack = bytes([0,1,2,2])
    com.sendData(syn) #Enviando Syn
    rxBuffer, nRx = com2.getData(syn) # Reconhecendo o Syn 
    if rxBuffer == syn:  # Se o rxBuffer estiver com o Syn!
        print("Server recebeu o Syn, vou te mandar o Ack")
        print("Transmitindo Ack para o client")
        com.sendData(syn) and com.sendData(ack) # Enviando syn e ack para o client
